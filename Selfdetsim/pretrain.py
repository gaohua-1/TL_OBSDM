...
from models import load_backbone, load_mlp, load_ss_predictor,resnet18,resnet50,resnet101
import trainers
from trainers import SSObjective
from utils import Logger
import sys
def detsim(args, t1, t2,t3,t4):
    out_dim = 2048
    device = idist.device()
    ...
    backbone     = resnet18(bn="vanilla").to(device)

    projector    = build_model(load_mlp(512,
                                        out_dim,
                                        out_dim,
                                        num_layers=2+int(args.dataset.startswith('imagenet')),
    #multi-projector                                        last_bn=True))
    pro1    = build_model(load_mlp(64,
                                        out_dim,
                                        out_dim,
                                        num_layers=2+int(args.dataset.startswith('imagenet')),
                                        last_bn=True))
    pro2    = build_model(load_mlp(128,
                                        out_dim,
                                        out_dim,
                                        num_layers=4,
                                        last_bn=True))
    pro3    = build_model(load_mlp(256,
                                        out_dim,
                                        out_dim,
                                        num_layers=2+int(args.dataset.startswith('imagenet')),
                                        last_bn=True))
    ...
    predictor    = build_model(load_mlp(out_dim,
                                        ...
                                        num_layers=2,
                                        last_bn=False))
    pre1        = build_model(load_mlp(out_dim,
                                        out_dim // 4,
                                        out_dim,
                                        num_layers=2,
                                        last_bn=False))
    pre2        = build_model(load_mlp(out_dim,
                                        out_dim // 4,
                                        out_dim,
                                        num_layers=2,
                                        last_bn=False))
    pre3        = build_model(load_mlp(out_dim,
                                        out_dim // 4,
                                        out_dim,
                                        num_layers=2,
                                        last_bn=False))

    ss_predictor = load_ss_predictor( 512,ss_objective)
    ss_predictor = { k: build_model(v) for k, v in ss_predictor.items() }
    ss_params = sum([list(v.parameters()) for v in ss_predictor.values()], [])
    ss_predictor1 = load_ss_predictor(64  ,ss_objective)
    ss_predictor1 = { k: build_model(v) for k, v in ss_predictor1.items() }
    ss_params1 = sum([list(v.parameters()) for v in ss_predictor1.values()], [])
    ss_predictor2 = load_ss_predictor(128  ,ss_objective)
    ss_predictor2 = { k: build_model(v) for k, v in ss_predictor2.items() }
    ss_params2 = sum([list(v.parameters()) for v in ss_predictor2.values()], [])
    ss_predictor3 = load_ss_predictor(256  ,ss_objective)
    ss_predictor3 = { k: build_model(v) for k, v in ss_predictor3.items() }
    ss_params3 = sum([list(v.parameters()) for v in ss_predictor3.values()], [])

    SGD = partial(optim.SGD, lr=args.lr, weight_decay=args.wd, momentum=args.momentum)
    build_optim = lambda x: idist.auto_optim(SGD(x))

    optimizers = [build_optim(list(backbone.parameters()) + list(projector.parameters()) + list(pro1.parameters()) + list(pro2.parameters()) + list(pro3.parameters())+ss_params+ss_params1+ss_params2+ss_params3),
                  build_optim(list(predictor.parameters()) + list(pre1.parameters()) + list(pre2.parameters()) + list(pre3.parameters()))]

    schedulers = [optim.lr_scheduler.CosineAnnealingLR(optimizers[0], args.max_epochs)]

    trainer = trainers.detsim(backbone=backbone,
                               projector=projector,
                               pro1=pro1,#multi projector
                               pro2=pro2,
                               pro3=pro3,
                               predictor=predictor,
                               pre1=pre1,
                               ...
                               ss_predictor1=ss_predictor1,
                               ss_predictor2=ss_predictor2,
                               ss_predictor3=ss_predictor3,
...
def main(local_rank, args):
    ...
    t1, t2 , t3,t4= datasets['t1'], datasets['t2'],datasets['t3'],datasets['t4']

    # MODELS
    if args.framework == 'simsiam':
        models = detsim(args, t1, t2,t3,t4)
    ...
    def log(engine):
        ...
        ss_loss = engine.state.output.pop('ss')
        ...
        def load_state(engine):
            ...
            for k, v in models.items():
                if isinstance(v, nn.parallel.DistributedDataParallel):
                    ...
                    #v = v(find_unused_parameters=True).module
                    v = v.module
